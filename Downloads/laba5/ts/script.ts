...
function CheckArmchairRed(flag:boolean) {
    ...
    if (flag) {
        for (let i = 0; i < this.productList.length; i++)
            if (productList[i] instanceof Armchair && (<Armchair>productList[i]).color=="красный")
                (<Armchair>productList[i]).Init();
    }
    else {
        for (let i = 0; i < this.productList.length; i++)
            productList[i].Init();
...
function CheckArmchairBig(flag:boolean) {
    ...
    if (flag) {
        for (let i = 0; i < this.productList.length; i++)
            if (productList[i] instanceof Armchair && (<Armchair>productList[i]).size=="большой")
                (<Armchair>productList[i]).Init();
    }
    else {
        for (let i = 0; i < this.productList.length; i++)
...
class MusicColumn extends Product {
    color: string;
    wireless: boolean;
    constructor(protected id: number, public name: string, public price: number, public description: string, public inStock: number, public col: string, public wrless:boolean) {
        super(id, name, price, description, inStock);
        this.color = col;
        this.wireless = wrless;
        this.Init();
    }
