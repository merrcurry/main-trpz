...
```
     * @example ​ ````Load the JSON data from script.js and access a name from the returned JSON data.
```javascript
$.getJSON( "script.js", function( json ) {
  console.log( "JSON Data: " + json.users[ 3 ].name );
 });
 ```
     * @example ​ ````Load the JSON data from script.js, passing along additional data, and access a name from the returned JSON data.
      If an error occurs, log an error message instead.
```javascript
$.getJSON( "script.js", { name: "John", time: "2pm" } )
  .done(function( json ) {
    console.log( "JSON Data: " + json.users[ 3 ].name );
  })
...
​
// Usage
$.cachedScript( "ajax/script.js" ).done(function( script, textStatus ) {
